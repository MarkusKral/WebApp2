...
test.describe('Sign up Page', function () {
  this.timeout(mochaTimeOut);
  test.before(function () {
    driver = new webdriver.Builder()
      .withCapabilities(webdriver.Capabilities.chrome())
      .build();
    pageSelector = By.id('signup');
  ...
  test.it('shows the main header', function () {
    driver.findElement(By.id('Headline')).then(function (element) {
      element.getText().then(function (text) {
        expect(text).to.equal('Please sign Up');
      });
    });
  });
  ...
  test.it('Sign up and list Receipes after', function () {
    ...
    select.then(function () {
      ...
      .then(function (element) {
        element.sendKeys('setupUserSelenium@testing.com');
      })
      .then(function () {
